...
function processLeeggoed() {
    const leeggoedEl2 = document.querySelector('p.leeggoed2');
    const leeggoedEl = document.querySelector('p.leeggoed');
    if (!leeggoedEl2) {
        if (leeggoedEl) {
            const leeggoedImgEl = leeggoedEl.nextElementSibling.firstChild;
            if (leeggoedImgEl) {
                const txt = leeggoedImgEl.outerHTML;
                if (txt.indexOf('<img') === 0) {
                    let imgWrapperEl = document.querySelector('div.details-gallery__main-image-wrapper');
                    if (imgWrapperEl) {
                        imgWrapperEl.insertAdjacentHTML('beforeend', '<p class="leeggoed2" style="display:inline-flex">' + leeggoedImgEl.outerHTML + '</p>');
                    }
                }
            }
        }
...
function showDeliveryInfo(countryCode) {
    ...
    if (countryCode === 'PICKUP' || countries.indexOf(countryCode) >= 0) {
        ...
        if (element) {
            ...
            const total = Number(document.querySelector('span.ec-cart-summary__total').textContent.split('(')[0].replace('€', '').replace(',','.').trim());
            let minColor = 'red';
            if (total >= minOrder) {
                minColor = 'green';
            }
...
function setServicePointWarnings() {
    if ('EN' === getCustomerLng()) {
        setServicePointWarning('BPost-postpunt-levering002fpostal-point-delivery', '!! Make sure you select a <img src="https://hopincraftbier.github.io/bpost.svg" height="40px" width="40px" style="margin:0 5px;"><strong>BPost service point</strong>, otherwise delivery may be delayed !!')
        setServicePointWarning('PostNL-postpunt-levering002fpostal-point-delivery', '!! Make sure you select a <img src="https://hopincraftbier.github.io/postnl.svg" height="25px" width="25px" style="margin:0 5px;"><strong>PostNL service point</strong>, otherwise delivery may be delayed !!')
    } else {
        setServicePointWarning('BPost-postpunt-levering002fpostal-point-delivery', '!! Zorg ervoor dat u een <img src="https://hopincraftbier.github.io/bpost.svg" height="40px" width="40px" style="margin:0 5px;"><strong>BPost-servicepunt</strong>&nbsp;selecteert, anders kan de levering vertraging oplopen !!')
        setServicePointWarning('PostNL-postpunt-levering002fpostal-point-delivery', '!! Zorg ervoor dat u een <img src="https://hopincraftbier.github.io/postnl.svg" height="25px" width="25px" style="margin:0 5px;"><strong>PostNL-servicepunt</strong>&nbsp;selecteert, anders kan de levering vertraging oplopen !!')
    }
}
...
function logCart(cart) {
    ...
    // even if every unit were the heaviest type, the cart couldn't exceed MAX
    if (cart.productsQuantity * heaviest <= MAX) return;

    Promise.all(cart.items.map(function (it) {
        return getVerpakking(it.product.id).then(function (verpakking) {
            return { verpakking: verpakking, quantity: it.quantity };
        });
    })).then(function (lines) {
        let blik = 0, fles = 0;

        lines.forEach(function (line) {
            if (line.verpakking === "Blik") blik += line.quantity;
            else if (line.verpakking === "Fles") fles += line.quantity;
        });
        ...
        console.log("Verpakking lookup failed", err);
    });
}
